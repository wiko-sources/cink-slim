...
#if 0
static void fiq_debug_ps(struct fiq_debugger_state *state)
{
	struct task_struct *g, *p;
	char p_state;

	debug_printf(state, "%-*s      Pid   Parent [*] cpu State %-*s Command\n",
	...
	do_each_thread(g, p) {
		...
		debug_printf(state, "0x%p %8d %8d  %d %4d   %c  0x%p %s\n",
			     (void *)p, p->pid, p->parent->pid,
			     task_curr(p), task_thread_info(p)->cpu,
			     p_state,
			     (void *)(&p->thread),
			     p->comm);
	} while_each_thread(g, p);
...
struct console fiq_debugger_console = {
	.name = "ttyFIQ",
