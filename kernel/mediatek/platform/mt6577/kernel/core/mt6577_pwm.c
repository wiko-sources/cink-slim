...
int mt6577_pwm_probe ( struct platform_device *pdev)
{
	platform_set_drvdata (pdev, pwm_dev);
	return RSUCCESS;
}

int  mt6577_pwm_remove(struct platform_device *pdev)
{
	if ( ! pdev ) {
	...
	PWMDBG ( "mt6577_pwm_remove\n" );
	return RSUCCESS;
}

void mt6577_pwm_shutdown(struct platform_device *pdev)
{
	printk("mt6577_pwm_shutdown\n");
	return;
}

struct platform_driver pwm_plat_driver={
	.probe = mt6577_pwm_probe,
	.remove = mt6577_pwm_remove,
	.shutdown = mt6577_pwm_shutdown,
	.driver = {
		.name="mt6577-pwm"
	},
};

static int __init mt6577_pwm_init(void)
{
	int ret;
...
static void __exit mt6577_pwm_exit(void)
{
	platform_device_unregister ( &pwm_plat_dev );
	platform_driver_unregister ( &pwm_plat_driver );
}

module_init(mt6577_pwm_init);
module_exit(mt6577_pwm_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("Changlei Gao <changlei.gao@mediatek.com>");
MODULE_DESCRIPTION(" This module is for mt6577 chip of mediatek");
